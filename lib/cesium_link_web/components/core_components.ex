defmodule CesiumLinkWeb.CoreComponents do
  @moduledoc """
  ...
  def input(%{type: "emoji"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      <.label for={@id}><%= @label %></.label>
      <div class="border-[0.666667px] mt-2 flex w-full rounded-lg border-zinc-300 py-2 pl-2 text-zinc-900 focus:border-zinc-400 focus:ring-0 phx-no-feedback:border-zinc-300 phx-no-feedback:focus:border-zinc-400 sm:text-sm sm:leading-6">
        <div id={@id} class="hover:cursor-pointer" phx-hook="EmojiPicker">
          <.icon name="hero-face-smile" class="text-zinc-800" />
        </div>
        <input id={"input-"<>@id} type={@type} name={@name} value={Phoenix.HTML.Form.normalize_value(@type, @value)} class="pl-2 outline-none hover:cursor-default" readonly {@rest} />
      </div>
      <.error :for={msg <- @errors}><%= msg %></.error>
    </div>
    """
  ...
  def table(assigns) do
    ...
    <div class="overflow-y-auto px-4 sm:overflow-visible sm:px-0">
      <table class="w-[40rem] mt-11 sm:w-full">
        ...
        <tbody id={@id} phx-update={match?(%Phoenix.LiveView.LiveStream{}, @rows) && "stream"} class="relative divide-y divide-zinc-100 border-t border-zinc-200 text-sm leading-6 text-zinc-700" {@rest}>
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(CesiumLinkWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(CesiumLinkWeb.Gettext, "errors", msg, opts)
    end
  end
